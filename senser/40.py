...
def distance_callback(sub_info):
    ...
    global t_detect, t_stop_command, response_time, start_time
    ...
    try:
        ...
        # บันทึกข้อมูลลง CSV (เฉพาะ time และ distance)
        with lock:
            if csv_writer:
                csv_writer.writerow([elapsed_time, tof_distance])
        
        # แสดงค่าระยะทาง
        print(f"Time: {elapsed_time:.3f}s, Distance: {tof_distance:.1f} cm")
        
        # ตรวจสอบเงื่อนไขหยุด
        if tof_distance < TARGET_DISTANCE and robot_moving:
            t_detect = time.time()
            ...
            print(f"Object detected! Distance: {tof_distance:.1f} cm")
...
if __name__ == '__main__':
    ...
    csv_path = os.path.join(current_dir, "raw.csv")
    
    # เปิดไฟล์ CSV สำหรับบันทึกข้อมูล
    with open(csv_path, mode="w", newline="", encoding="utf-8") as csvfile:
        csv_writer = csv.writer(csvfile)
        csv_writer.writerow(["Time", "Distance"])  # Header: เฉพาะ Time และ Distance
        ...
        try:
            ...
            print(f"Will stop when distance < {TARGET_DISTANCE} cm")
